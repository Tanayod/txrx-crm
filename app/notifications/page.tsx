            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-3">
                <select value={certStatus} onChange={(e) => setCertStatus(e.target.value as any)}
                  className="border border-gray-200 rounded-lg px-2.5 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-[#4338CA]">
                  <option value="ทั้งหมด">ทั้งหมด</option>
                  <option value="ยังไม่ส่ง">ยังไม่ส่ง</option>
                  <option value="ส่งแล้ว">ส่งแล้ว</option>
                </select>
                <button onClick={goToday} className="text-xs text-[#4338CA] hover:underline">วันนี้</button>
              </div>
            </div>
                ...
                return (
                  <button key={i} onClick={() => setSelectedDate(dateStr)}
                    className={`aspect-square rounded-lg p-1.5 flex flex-col items-center justify-start text-left transition-all
                      ${isSelected ? 'ring-2 ring-[#4338CA]' : ''}
                      ${color ? color.bg : 'bg-gray-50 hover:bg-gray-100'}
                    `}>
                    <span className={`text-xs font-medium ${isToday ? 'bg-[#4338CA] text-white rounded-full w-5 h-5 flex items-center justify-center' : color ? color.text : 'text-gray-400'}`}>
                      {dayNum}
                    </span>
                    {dayCases.length > 0 && (
                  ...
                  <div className="flex items-center justify-between mt-2">
                    ...
                    {c.delivered ? (
                      ...
                      <button onClick={() => handleMarkDelivered(c.mc_id)}
                        className="text-xs bg-[#4338CA] text-white px-2.5 py-1 rounded-lg hover:bg-[#312E81] flex items-center gap-1">
            ...
            customerList.map((c: any, i) => (
              <div key={i} className="border-b border-gray-50">
                <div className="grid grid-cols-7 gap-2 px-5 py-3 text-sm hover:bg-gray-50 items-center">
                  ...
                  <button onClick={() => window.location.href='/customers'} className="text-xs text-[#4338CA] hover:underline text-right">ดูลูกค้า</button>
                </div>
