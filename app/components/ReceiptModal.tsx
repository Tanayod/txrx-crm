        ...
        <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="p-5 space-y-3">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <select value={issuerSignatureId}
                    ...
                    className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#4338CA]">
                    <option value="">-- ไม่มีลายเซ็น (เซ็นสด) --</option>
                    {signatures.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                  </select>
                  <button type="button" onClick={() => setAddingFor(addingFor === 'issuer' ? null : 'issuer')}
                    className="px-3 py-2 text-xs border border-[#4338CA] text-[#4338CA] rounded-lg hover:bg-blue-50 whitespace-nowrap">
                    + เพิ่มใหม่
                  </button>
                </div>
                {!issuerSignatureId && (
                  <input value={issuerName} onChange={(e) => setIssuerName(e.target.value)} placeholder="หรือพิมพ์ชื่อเฉยๆ (ไม่มีรูปลายเซ็น)"
                    className="w-full mt-1.5 border border-gray-200 rounded-lg px-3 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-[#4338CA]"/>
                )}
                {addingFor === 'issuer' && (
                  <div className="mt-2 p-2.5 bg-white border border-blue-200 rounded-lg space-y-2">
                    ...
                    <button type="button" onClick={handleAddSignature} disabled={savingSignature || !newSigName || !newSigFile}
                      className="w-full py-1.5 text-xs bg-[#4338CA] text-white rounded-lg hover:bg-[#312E81] disabled:opacity-50">
            ...
            <div className="bg-blue-50 rounded-lg p-3 space-y-1">
              <div className="flex justify-between items-center">
                ...
                <span className="text-lg font-bold text-[#4338CA]">฿{totalAmount.toLocaleString(undefined, {minimumFractionDigits:2, maximumFractionDigits:2})}</span>
              </div>
              {whtAmount > 0 && (
                <>
          ...
          <div className="p-4 border-t border-gray-100 flex justify-end gap-2">
            ...
            <button onClick={handleConfirmIssue} disabled={saving}
              className="px-5 py-2 text-sm bg-[#4338CA] text-white rounded-lg hover:bg-[#312E81] disabled:opacity-50 font-medium">
      ...
      <div className="bg-white rounded-2xl w-full max-w-3xl shadow-2xl max-h-[95vh] overflow-y-auto print:max-w-full print:shadow-none print:rounded-none print:max-h-full">
        <div className="no-print p-4 border-b border-gray-100 flex justify-between items-center sticky top-0 bg-white z-10">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handlePrint} className="flex items-center gap-1 px-3 py-1.5 text-xs bg-[#4338CA] text-white rounded-lg hover:bg-[#312E81]">
