...
import { IconSearch, IconDownload, IconRefresh, IconMicroscope } from '@tabler/icons-react'
...
export default function FinanceReport() {
  ...
  async function fetchReport() {
    ...
    while (true) {
      let q = supabase.from('bookings')
        .select('id, case_number, booking_date, booked_count, location_name, customers(customer_name, type), medical_cases(actual_count), payments(id, total_amount, amount_received, payment_status, method, invoice_no, ref_no, wht_amount, credit_used, credit_deposited), special_exams(*, special_exam_items(*))')
    ...
    const mapped = all.map((b: any) => {
      ...
      const specialAmount = b.special_exams?.reduce((s: number, e: any) => s + (e.total_amount || 0), 0) || 0
      ...
      return {
        ...
        special_workers: specialWorkers,
  ...
  const summary = {
    ...
    totalSpecialWorkers: filtered.reduce((s, r) => s + r.special_workers, 0),
    totalSpecialAmount: filtered.reduce((s, r) => s + r.special_amount, 0),
  ...
  const exportExcel = () => {
    const exRows = filtered.map(r => ({
      ...
      'ตรวจพิเศษ (คน)': r.special_workers,
      'ยอดตรวจพิเศษ (บาท)': r.special_amount,
  ...
  return (
    <div className="flex min-h-screen bg-[#F1F5F9]">
      ...
      <div className="flex-1 ml-56 p-6">
        ...
        {/* 🔹 ตรวจพิเศษ / หัก ณ ที่จ่าย / เครดิต Summary */}
        <div className="grid grid-cols-3 gap-3 mb-4">
          <div className="bg-purple-50 border border-purple-100 rounded-xl p-3 flex justify-between items-center">
            <span className="text-sm text-purple-700 font-medium flex items-center gap-1">
              <IconMicroscope size={15}/> ยอดตรวจพิเศษรวม ({summary.totalSpecialWorkers} คน)
            </span>
            <span className="text-lg font-bold text-purple-700">฿{fmt(summary.totalSpecialAmount)}</span>
          </div>
          <div className="bg-rose-50 border border-rose-100 rounded-xl p-3 flex justify-between items-center">
            <span className="text-sm text-rose-600 font-medium">💸 หัก ณ ที่จ่าย 3% รวม</span>
          ...
          <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3 flex justify-between items-center">
            <span className="text-sm text-emerald-600 font-medium">💚 เครดิตที่ใช้หักยอดรวม</span>
        ...
        <div className="bg-white border border-gray-100 rounded-xl overflow-hidden shadow-sm">
          ...
          {loading ? (
            ...
            <div key={r.id} className={`grid grid-cols-12 gap-2 px-4 py-3 border-b border-gray-50 text-xs hover:bg-blue-50/20 items-center ${i % 2 !== 0 ? 'bg-gray-50/30' : ''}`}>
              ...
              <span>
                {r.special_workers > 0 ? (
                  <span className="text-purple-700 font-medium bg-purple-50 px-1.5 py-0.5 rounded">
                    {r.special_workers}คน (฿{fmt(r.special_amount)})
                  </span>
          ...
          {filtered.length > 0 && (
            <div className="grid grid-cols-12 gap-2 px-4 py-3 bg-gray-800 text-xs font-bold text-white rounded-b-xl">
              <div className="col-span-2">รวม {filtered.length} รายการ</div>
              <div className="col-span-2"></div>
              <span></span>
              <span className="text-purple-300">{summary.totalSpecialWorkers > 0 ? `${summary.totalSpecialWorkers} คน` : '-'}</span>
